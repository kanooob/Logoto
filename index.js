...
(async () => {
    ...
    s4d.client.on('interactionCreate', async (interaction) => {
        if ((interaction.commandName) == 'setup' && (interaction.guild).channels.cache.find((category) => category.name === 'log-logoto') !== undefined && ((((interaction.member).roles.highest).permissions.has('MANAGE_GUILD')) || (((interaction.member).roles.highest).permissions.has('ADMINISTRATOR')) || (String((interaction.guild).ownerId)) == ((interaction.member).id))) {
            ...
            (interaction.guild).channels.create('Logoto', { type: 'GUILD_CATEGORY' }).then(async cat => { 
                (interaction.guild).channels.create('log-logoto', { type: "GUILD_TEXT", parent: (cat) }).then(async cat => { 
                    (cat).permissionOverwrites.edit((s4d.client.users.cache.get(String('1431383390162124920'))), { VIEW_CHANNEL: true });
                    (cat).permissionOverwrites.edit(((interaction.guild).roles.cache.get(((interaction.guild).id))), { VIEW_CHANNEL: false });
                    ...
                    (cat).messages.fetch({ limit: 1 }).then(async (last_messages_in_channel) => {
                        (last_messages_in_channel.at(0)).pin();
                    });
        ...
        if ((interaction.commandName) == 'add-an-event' && ((((interaction.member).roles.highest).permissions.has('MANAGE_GUILD')) || (((interaction.member).roles.highest).permissions.has('ADMINISTRATOR')) || (String((interaction.guild).ownerId)) == ((interaction.member).id))) {
            (interaction.guild).channels.create(([interaction.options.getString('type'), '-', interaction.options.getInteger('day'), '-', interaction.options.getInteger('month')].join('')), { type: "GUILD_TEXT", parent: (interaction.guild).channels.cache.find((category) => category.name === 'Logoto') }).then(async cat => { 
                (cat).permissionOverwrites.edit((s4d.client.users.cache.get(String('1431383390162124920'))), { VIEW_CHANNEL: true });
                (cat).permissionOverwrites.edit(((interaction.guild).roles.cache.get(((interaction.guild).id))), { VIEW_CHANNEL: false });
    ...
    s4d.client.on('guildCreate', async (s4dguild) => {
        ...
        ((s4d.client.guilds.cache.get((s4dguild.id))).systemChannel).messages.fetch({ limit: 1 }).then(async (last_messages_in_channel) => {
            (s4d.client.users.cache.get(String((String((s4d.client.guilds.cache.get((s4dguild.id))).ownerId))))).send({ content: String((['<:loudspeaker:1505590722798293253> **I thank the ', s4dguild.name, ' Discord server team.**', '\n', '<:track_next:1505295937856213072> For more information, please visit my website and my support server.', '\n', '<:track_next:1505295937856213072> Pour plus d\'information vous avez mon site internet et mon serveurs de support.', '\n', '-# <:link:1505215573364047913> [Website](https://logoto.onrender.com/index-en), [Support server](https://discord.gg/TPXFVYVnXe), https://discord.com/channels/', s4dguild.id, '/', ((last_messages_in_channel.at(0)).channel).id, '/', (last_messages_in_channel.at(0)).id].join(''))) });
        });
    });
    
    s4d.client.on('messageCreate', async (s4dmessage) => {
        if (s4dmessage.guild && (s4dmessage.guild).channels.cache.find((category) => category.name === (['l-', (new Date().getDate()), '-', ((new Date().getMonth())) + 1].join(''))) && ((s4dmessage).content) == '<:loop:1505215574387589162> l-Loading') {
        ...
        if (s4dmessage.guild && (s4dmessage.guild).channels.cache.find((category) => category.name === (['n-', (new Date().getDate()), '-', ((new Date().getMonth())) + 1].join(''))) && ((s4dmessage).content) == '<:loop:1505215574387589162> n-Loading') {
        ...
        if (s4dmessage.guild && (s4dmessage.guild).channels.cache.find((category) => category.name === (['b-', (new Date().getDate()), '-', ((new Date().getMonth())) + 1].join(''))) && ((s4dmessage).content) == '<:loop:1505215574387589162> b-Loading') {
        ...
        if (s4dmessage.guild && (s4dmessage.guild).channels.cache.find((category) => category.name === (['i-', (new Date().getDate()), '-', ((new Date().getMonth())) + 1].join(''))) && ((s4dmessage).content) == '<:loop:1505215574387589162> i-Loading') {
