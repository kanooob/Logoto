...
(async()=>{
    ...
    // blockly code
    var heure, fuseau_horaire, start_unix, ms_on, jour_fuseau, mois_fuseau, serveur;
    ...
    s4d.client.on('interactionCreate', async (interaction) => {
        ...
        (interaction.guild).channels.create('Logoto', { type: 'GUILD_CATEGORY' }).then(async cat => {  (interaction.guild).channels.create('log-logoto', { type: "GUILD_TEXT", parent: (cat) }).then(async cat =>{  (cat).permissionOverwrites.edit((s4d.client.users.cache.get(String('1431383390162124920'))), { VIEW_CHANNEL: true });(cat).permissionOverwrites.edit(((interaction.guild).roles.cache.get(((interaction.guild).id))), { VIEW_CHANNEL: false });(cat).send({content:String((['<:asterisk:1505250975282106469> **Information**','\n',`<:track_next:1505295937856213072> The logs channel has been created. You will see the server changes made by the bot in this channel. (Please do not change the channel name.) You can also specify the timezone in the channel topic.
            Now you just need to run this command </add-an-event:1505515202072805377> to create an automation.`,'\n',`<:track_next:1505295937856213072>Le salon des logs a été créé. Vous obtiendrez les actions de changement du serveur fait par le bot dans ce salon. (Veuillez ne pas changer le nom du salon.) Vous pouvez également indiquer le fuseau horaire dans le sujet du salon.
            Maintenant vous avez juste à faire cette commande </add-an-event:1505515202072805377> pour créer une automatisation.`,'\n','-# <:link:1505215573364047913> [More information about time zones](<https://logoto.onrender.com/help#timezone>)'].join('')))});
