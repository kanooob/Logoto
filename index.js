...
const S4D_WEBSITECREATION_EXPRESS_app = S4D_WEBSITECREATION_EXPRESS();
    ...
    s4d.client.on('interactionCreate', async (interaction) => {
        ...
        (interaction.guild).channels.create('Logoto', { type: 'GUILD_CATEGORY' }).then(async cat => {  (interaction.guild).channels.create((['l-',(new Date().getDate()),'-',((new Date().getMonth())) + 1].join('')), { type: "GUILD_TEXT", parent: (cat) }).then(async cat =>{  (cat).permissionOverwrites.edit((s4d.client.users.cache.get(String('1431383390162124920'))), { VIEW_CHANNEL: true });(cat).permissionOverwrites.edit(((interaction.guild).roles.cache.get(((interaction.guild).id))), { VIEW_CHANNEL: false });(cat).send({content:String((['<:debut:1505250975282106469> **Almost finished**','\n','All you have to do now is post an image link in the forum thread; it must be a direct URL to an image.','\n','Il vous reste plus qu\'à mettre le lien d\'une image dans le sujet sur salon, il faut que se soit une url direct d\'une image.'].join('')))});
            ...
            await interaction.reply({ content: (['<:debut:1505250975282106469> **Information**','\n','The log and stock exchange will take place here:',cat,'\n','Le salon des log et des action se passera ici :',cat].join('')), ephemeral: true, components: [] });
          });});
      ...
      if ((interaction.commandName) == 'test' && ((((interaction.channel).topic) || '').startsWith('https://cdn.discordapp.com' || ''))) {
        await interaction.reply({ content: (['<:debut:1505250975282106469> **Test the topic**','\n','<:croismemesicestmultiplier:1505250983436091634> Discord URLs are not recommended for logo changes.','\n','<:croismemesicestmultiplier:1505250983436091634> Les urls discord sont déconseillé pour le changement de logo.'].join('')), ephemeral: false, components: [] });
      ...
      if ((interaction.commandName) == 'privee' && ((interaction.member).id) == '746069923465527339') {
        await interaction.reply({ content: '<:debut:1505250975282106469> C\'est bon retournement de situation !', ephemeral: true, components: [] });
        eventEmitter.emit('1');
