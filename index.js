(async()=>{
    ...
    eventEmitter.on('message1', async => {
        ...
        (s).channels.cache.find((category) => category.name === ([(new Date().getDate()),'-',((new Date().getMonth())) + 1].join(''))).send({content:String(([`✅ **Le logo du serveur à été mis à jour !**
        Action :Changer le logo du serveur.
        Date :`,[(new Date().getDate()),'-',((new Date().getMonth())) + 1].join(''),`
        Nouveau Logo :`,(s).channels.cache.find((category) => category.name === ([(new Date().getDate()),'-',((new Date().getMonth())) + 1].join(''))).topic].join('')))});
